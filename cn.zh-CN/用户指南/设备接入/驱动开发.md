...
设备接入SDK支持的三种开发语言详细内容，请参见[C](../../../../../cn.zh-CN/边缘开发指南/设备接入API/C版本SDK.md#)、[Node.js](../../../../../cn.zh-CN/边缘开发指南/设备接入API/Nodejs版本SDK.md#)和[Python](../../../../../cn.zh-CN/边缘开发指南/设备接入API/Python版本SDK.md#)。

## 背景信息 {#section_wfq_r5y_pgb .section}

本驱动示例实现如下场景：通过自定义驱动，实现一个LED灯设备接入Link IoT Edge，您可以通过物联网平台控制台控制LED灯设备的开关操作，也可以主动查询灯设备的当前温度。

![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892487838383_zh-CN.png)
...
在开始操作本章内容前，请您确保已根据[基于Ubuntu 16.04搭建专业版环境](cn.zh-CN/用户指南/环境搭建/标准版环境搭建/基于Ubuntu 16.04搭建环境.md#)内容完成边缘实例的创建，为C语言SDK开发的设备驱动准备运行环境。
...
2.  解析驱动配置，完成设备上线。通过leda\_get\_config\_size接口和leda\_get\_config接口获取设备配置，按照驱动配置格式解析，根据解析后的驱动配置连接设备并调用leda\_register\_and\_online\_by\_device\_name接口完成设备注册上线到[阿里云物联网平台](https://iot.console.aliyun.com/)。
    ...
    驱动配置格式定义请参考[驱动开发注意事项\>驱动配置](https://yuque.antfin-inc.com/aone683673/link-iot-edge-doc/rqhm74)章节。
...
1.  编译打包。
    1.  下载示例代码。
        ...
        ```
        git clone https://github.com/aliyun/link-iot-edge-access-sdk-c.git
        ```

    2.  进入link-iot-edge-access-sdk-c目录，完成示例编译。
    ...
    3.  查看示例编译产物。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892487937957_zh-CN.png)

2.  上传驱动。
    1.  在[物联网平台控制台](https://iot.console.aliyun.com/)左侧导航栏中，单击**边缘计算** \> **驱动管理**。
    2.  在驱动管理页面，单击右侧**新增驱动**。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892487938387_zh-CN.png)

    3.  在**新增驱动**界面，根据提示设置驱动参数，上传已完成编译并打包的demo\_led.zip文件。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892487938388_zh-CN.png)

    4.  完成参数的设置并上传成功驱动文件后，单击确认。您可以在驱动管列表中看到刚刚创建的驱动。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892487938389_zh-CN.png)

3.  创建产品。
    1.  参考[创建产品\(高级版\)](../../../../../cn.zh-CN/用户指南/产品与设备/创建产品(高级版).md#)，创建一个产品。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892487938390_zh-CN.png)

    2.  参考[新增物模型](../../../../../cn.zh-CN/用户指南/产品与设备/物模型/新增物模型.md#)，为产品添加属性。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892487938391_zh-CN.png)

    3.  参考[新增物模型](../../../../../cn.zh-CN/用户指南/产品与设备/物模型/新增物模型.md#)，为产品添加事件。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892487938392_zh-CN.png)

        为事件增加参数。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892487938393_zh-CN.png)

4.  添加设备。

    参考[单个创建设备](../../../../../cn.zh-CN/用户指南/产品与设备/创建设备/单个创建设备.md#)，为产品添加设备。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892487938394_zh-CN.png)
...
5.  部署驱动。
    ...
    3.  在分配子设备页面中，将之前创建的产品和设备分配至实例中，单击设备右侧的**分配**。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892487938395_zh-CN.png)

    4.  单击**完成**，完成设备的分配。
    5.  单击已分配的子设备右侧的**驱动配置**，为设备分配[上传驱动](#)步骤中完成的驱动。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892487938396_zh-CN.png)

    6.  在实例详情页面，选择消息路由，单击**添加路由**。
    7.  在添加消息路由页面中，配置参数，您可以通过[设置消息路由](cn.zh-CN/用户指南/消息路由/设置消息路由.md#)查看具体配置方式。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892487938399_zh-CN.png)
...
6.  查看调试信息。
    ...
    1.  在实例详情页面，实例详情页签下查看设备连接状态。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892488038401_zh-CN.png)

    2.  在物联网平台，选择**设备管理** \> **设备**，单击led\_device设备右侧操作栏中的**查看**，进入设备详情页面，选择运行状态查看设备上报的数据。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892488038403_zh-CN.png)

    3.  在物联网平台，选择**监控运维** \> **日志服务**，在**日志服务**页面，**产品**参数后选择led\_demo产品，查看**设备行为分析**。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892488038405_zh-CN.png)

    4.  在**日志服务**页面，选择**上行消息分析**查看设备数据上报日志。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892488038406_zh-CN.png)
    ...
    1.  通过`./fctl show`命令，可以查看到部署驱动的具体位置。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892488038407_zh-CN.png)
    ...
    2.  驱动在运行过程中会产生运行日志，通过查看运行日志可以了解驱动运行状态，每个自定义驱动生成的日志文件统一放在/linkedge/run/logger/fc-base路径下。每个驱动日志文件路径为/linkedge/run/logger/fc-base/xxxx/xxxx.txt。
        ...
        **说明：** 驱动日志文件路径中的xxxx为在上传驱动时填写的名称。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892488038411_zh-CN.png)
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892488038412_zh-CN.png)
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892488038413_zh-CN.png)
...
7.  调试数据交互。
    ...
    1.  在物联网平台，选择**监控运维** \> **在线调试**，在**在线调试**页面，选择调试产品和调试设备。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892488038415_zh-CN.png)

    2.  选择调试设备待调功能和方法，进行调试，并查看设备实时运行日志。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892488138416_zh-CN.png)
...
8.  本地替换驱动。
    ...
    1.  找到驱动的位置，可以通过fctl命令进行查找，具体字段解释查看调试信息。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/117524/154892488138417_zh-CN.png)
